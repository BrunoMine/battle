...
-- Tradução intllib
battle.intllib = {}
battle.intllib.S, battle.intllib.NS = dofile(modpath.."/lib/intllib.lua")

-- Configura tradutor opicional
battle.S = battle.intllib.S
battle.NS = battle.intllib.NS
...
-- Gera arquivos de tradução battle.*.tr
do
	...
	for _,file in ipairs(list) do
		...
		if string.match(file, "~") == nil then
			...
			if string.match(file, ".pot") == nil and string.match(file, ".po") then
				...
				-- Novo arquivo
				local new_file = "### Arquivo gerado por battle apartir de "..file.."\n# textdomain: battle\n"
				for en,lang in pairs(en_to_lang) do
					new_file = new_file .. en .. "=" .. lang .. "\n"
				end
				-- Escrever arquivo
				local saida = io.open(modpath.."/locale/battle."..lang_code..".tr", "w")
...
if minetest.get_translator ~= nil then
	s = minetest.get_translator("battle")
else
	s = battle.intllib.S
end
...
battle.s = function(...)
	...
	minetest.log("error", "[battle] String "..dump(args[1]).." nao catalogada")
	return s(...)
end
	...
	if type(args[1]) == "table" then
		...
		for n,a in ipairs(args[1]) do
			if n ~= 1 then -- Não traduz o primeiro
				table.insert(r, battle.S(a))
			else
				table.insert(r, a)
			end
		end
		
		return battle.s(unpack(r))
...
battle.Sfake = function(s) return s end
